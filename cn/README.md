...
当新资产上市时，整个流动性池都会暴露在其中，这意味着使用新抵押资产的用户可能会借用整个可用的流动性。这大大限制了协议添加新资产的能力，因为每一个新资产都会增加流动性和偿付能力风险。协议通过隔离模式解决这个问题。

可以通过治理将协议中没有供应量的资产设置为隔离资产。任何时候可以通过治理将隔离资产设置为普通资产。隔离资产有一个特定的债务上限。使用隔离资产作为抵押品的借款人只能使用该特定资产作为抵押品，而不能启用任何其他资产（包括其他隔离资产）。隔离资产只能借出稳定币资产。通过设置隔离资产作为抵押品，进入隔离模式。通过禁止隔离资产作为抵押品，退出隔离模式。

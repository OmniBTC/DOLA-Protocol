...
风险调整：不同于传统借贷，仅仅考虑用户抵押品价值降低带来的坏账风险。协议同时考虑负债价值上升带来的风险，通过 $BF$(大于1)将负债价值提升来抵御这种风险。 $CF$(小于1)用于降低抵押品价值。$VC_i$用于表示抵押$i$的价值， $VD_i$用于表示负债$i$的价值。 当用户的抵押品价值 $TotalCollateral$和负债价值 $TotalDebt$不满足如下公式时，会被清算。
...
清算折扣: 基础折扣$BDC$是清算人的基础折扣, 实际折扣$DC$是清算人实际清算折扣，最大折扣MDC是最大可能的折扣，国库折扣$TDC$是保留给国库的折扣。$AL$代表用户平均流动性， $\Delta T$ 代表当前时间到上一次更新的间隔时间, $DAY$代表一天时间。$Min$用于取较小值。
...
清算数量: 目标健康因子$TH$代表清算结束期望达到的健康因子。 $TC$代表当前抵押品总价值,  $TD$代表当前负债总价值。 $PC_i$代表当前抵押品$i$价格，$PD_i$代表当前负债$i$价格。 $LC_i$用于表示抵押品$i$的最大清算价值， $LD_i$用于表示负债$i$的最大清算价值。 $TCL_i$代表实际清算的抵押品$i$最大数量， $TDL_i$代表实际清算的负债$i$最大数量。 $TCU_i$代表用户能够得到的抵押品$i$最大数量， $TDT_i$代表保留给国库的抵押品$i$数量

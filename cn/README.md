...
风险调整：不同于传统借贷，仅仅考虑用户抵押品价值降低带来的坏账风险。协议同时考虑负债价值上升带来的风险，通过 $BF$ (大于1)将负债价值提升来抵御这种风险。$CF$(小于1)用于降低抵押品价值。 $VC_i$ 用于表示抵押i的价值， $VD_i$ 用于表示负债i的价值。当用户的抵押品总价值 $TC$ 和负债总价值 $TD$ 不满足如下公式时，会被清算。
...
软性清算：在传统借贷中，清算人一次性清算的债务是固定的，目前是0.5，即清算人一次性允许清算人清算一半的债务。这种方式的缺点是如果较小的清算可以使债务恢复健康，那么清算一半的债务是过度的且不公平的。因此，本协议将采用软性清算模式，每次允许清算的债务不超过使违约者恢复目标健康因子所需的金额。这意味着轻微违约的借款人清算的债务少于一半，严重违约的借款人超过一半。
...
清算数量: 目标健康因子 $TH$ 代表清算结束期望达到的健康因子。 $TC$ 代表当前抵押品总价值, $TD$ 代表当前负债总价值。 $PC_i$ 代表当前抵押品 $i$ 价格， $PD_i$ 代表当前负债i价格。 $LC_i$ 用于表示抵押品 $i$ 的最大清算价值， $LD_i$ 用于表示负债i的最大清算价值。 $TCL_i$ 代表实际清算的抵押品 $i$ 最大数量， $TDL_i$ 代表实际清算的负债 $i$ 最大数量。 $TCU_i$ 代表用户能够得到的抵押品i最大数量， $TDT_i$ 代表保留给国库的抵押品 $i$ 数量
...
清算过程：通过公式得到最大可清算的抵押品和负债之后，清算人通过支付负债就能以一定的折扣拿到相应的抵押品。
